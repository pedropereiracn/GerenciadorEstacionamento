...
// mensalista -> puxa a ficha pela placa. avulso -> gera ticket e paga por tempo na saida.
public class AbaEntrada extends JPanel {
    ...
    private JLabel resultado;   // mostra a ficha ou o ticket gerado
    ...
    private JComponent cardPatio() {
        ...
        // linha de saida: numero do ticket + botao
        numTicket = campo();
    ...
    private void entrar() {
        ...
        // 2) nao achou -> avulso -> gera ticket
        Veiculo v = btCarro.isSelected()
                ? new Carro(placaFmt, "Avulso", 0)
                : new Moto(placaFmt, "Avulso", 0);
        Ticket t = new Ticket(proximoTicket++, v);
        tickets.add(t);
        atualizarPatio();
        mostrarResultado(String.format(
                "<html>🎫 <b>Avulso</b> — ticket #%03d gerado.<br>Paga por tempo na saída.</html>",
                t.getNumero()), ACENTO);
        placa.setText("");
    }
    ...
    private void registrarSaida() {
        ...
        for (Ticket t : tickets) {
            if (t.getNumero() == num && t.estaAberto()) {
                long min = t.registrarSaida();
                atualizarPatio();
                numTicket.setText("");
                feedback(String.format("✓ Saída #%03d — %d min — R$ %.2f",
                        num, min, t.valor()), VERDE);
                return;
            }
        }
